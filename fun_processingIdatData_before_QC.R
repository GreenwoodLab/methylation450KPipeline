processingIdatData_before_QC <- function(path, server){
#load library
  if (server=="distant"){
  LIB_METH = NULL
  }else{
  ...
  require(IlluminaHumanMethylation450kmanifest, lib.loc = LIB_METH)
  require(IlluminaHumanMethylation450kanno.ilmn12.hg19, lib.loc = LIB_METH)
  require(matrixStats, lib.loc = LIB_METH)
  require(shinyMethyl, lib.loc = LIB_METH)
  require(shinyMethylData, lib.loc = LIB_METH)
